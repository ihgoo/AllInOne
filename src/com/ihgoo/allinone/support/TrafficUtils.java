...
import com.ihgoo.allinone.BuildConfig;
...
public abstract class TrafficUtils {
    ...
    public static long start(Context context, String tag) {
        ...
        if (uid > 0) {
            ...
            if (DEBUG) {
                LogUtils.v("start() rxValue=" + appRxValue / 1000 + " txValue=" + appTxValue / 1000 + " uid=" + uid);
            }
            return appRxValue;
        }
        return 0;
    ...
    public static long current(Context context, String tag) {
        ...
        if (appRxValue == null || appRxValue == null) {
            if (DEBUG) {
                LogUtils.w("current() appRxValue or appTxValue is null.");
            }
            return 0;
        }
        final int uid = getUid(context);
        ...
        if (DEBUG) {
            LogUtils.v("current() rxValue=" + rxValue / 1000 + " txValue=" + txValue / 1000 + " uid=" + uid);
        }
        return rxValue;
    ...
    public static long stop(Context context, String tag) {
        ...
        if (appRxValue == null || appRxValue == null) {
            if (DEBUG) {
                LogUtils.w("stop() appRxValue or appTxValue is null.");
            }
            return 0;
        }
        final int uid = getUid(context);
        ...
        long txValue = appTxValue2 - appTxValue;
        if (DEBUG) {
            LogUtils.v("stop() rxValue=" + rxValue / 1000 + " txValue=" + txValue / 1000 + " uid=" + uid);
        }
        return rxValue;
